...
void rpkt_event_continuum(Packet& pkt, const ContinuumOpacity& chi_rpkt_cont) {
  ...
  if (chi_rnd < chi_escatter) {
    ...
    const auto chi_bf_sum_span = std::span{phixslist.chi_bf_sum};
    ...
    const auto allcontindex =
        std::ranges::upper_bound(
            chi_bf_sum_span.subspan(phixslist.allcontbegin, phixslist.allcontend - phixslist.allcontbegin - 1),
            chi_bf_rand) -
        chi_bf_sum_span.begin();
