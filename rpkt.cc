...
// The caller moves the packet to the middle of the path increment, so pkt and doppler give the values there.
void update_estimators(const Packet& pkt, const double doppler, const double distance, const int nonemptymgi,
...
auto do_rpkt_step(Packet& pkt, const double t2, ContinuumOpacity& chi_rpkt_cont) -> bool {
  ...
  if ((edist < boundarydist) && (edist <= tdist)) [[likely]] {
    ...
    const auto doppler = move_pkt_withtime(pkt, edist / 2.);
    update_estimators(pkt, doppler, edist, nonemptymgi, chi_rpkt_cont, thickcell);
  ...
  if ((boundarydist <= tdist) && (boundarydist <= edist)) {
    ...
    const auto doppler = move_pkt_withtime(pkt, boundarydist / 2.);
    if (nonemptymgi >= 0) {
      update_estimators(pkt, doppler, boundarydist, nonemptymgi, chi_rpkt_cont, thickcell);
    }
  ...
  if ((tdist < boundarydist) && (tdist <= edist)) [[unlikely]] {
    ...
    const auto doppler = move_pkt_withtime(pkt, tdist / 2.);
    if (nonemptymgi >= 0) {
      update_estimators(pkt, doppler, tdist, nonemptymgi, chi_rpkt_cont, thickcell);
    }
