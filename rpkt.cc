...
auto get_possible_event_expansion_opacity(const int nonemptymgi, const Packet& pkt,
  ...
  assert_always(globals::cellcache[nonemptymgi].nonemptymgi == nonemptymgi);
  double dist = 0.;
  double tau = 0.;
...
auto calculate_chi_ffheating(const int nonemptymgi, const double nu, const bool use_cellcache) -> double {
  ...
  assert_testmodeonly(!use_cellcache || globals::cellcache[nonemptymgi].nonemptymgi == nonemptymgi);
  const auto chi_ff_nnionpart =
      use_cellcache ? globals::cellcache[nonemptymgi].chi_ff_nnionpart : calculate_chi_ffheat_nnionpart(nonemptymgi);
...
auto calculate_chi_bf_gammacontr(const int nonemptymgi, const double nu, Phixslist& phixslist) -> double {
  ...
  for (int i = allcontbegin; i < allcontend; i++) {
    ...
    const bool should_keep_this_cont = USECELLHISTANDUPDATEPHIXSLIST
                                           ? globals::cellcache[nonemptymgi].allcont_keep[i]
                                           : keep_this_cont(element, ion, level, nonemptymgi, nnetot);

    if (should_keep_this_cont) [[likely]] {
      const double nnlevel = USECELLHISTANDUPDATEPHIXSLIST ? globals::cellcache[nonemptymgi].allcont_nnlevel[i]
      ...
      if (USECELLHISTANDUPDATEPHIXSLIST || nnlevel > 0) {
        ...
        double modified_departure_ratio = globals::cellcache[nonemptymgi].allcont_modified_departureratios[i];
        ...
        if (!USECELLHISTANDUPDATEPHIXSLIST || modified_departure_ratio < 0) {
          ...
          if (USECELLHISTANDUPDATEPHIXSLIST) {
            globals::cellcache[nonemptymgi].allcont_modified_departureratios[i] = modified_departure_ratio;
          }
        }
