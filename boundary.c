     ...
     //   double *closest;
{
  ...
  tx_minus = ((x0 - (vx * tstart))/(cellxmin - (vx * tmin)) * tmin) - tstart;

  ty_plus = ((y0 - (vy * tstart))/(cellymax - (vy * tmin)) * tmin) - tstart;
  ty_minus = ((y0 - (vy * tstart))/(cellymin - (vy * tmin)) * tmin) - tstart;
     ...
     double t_current;
{
  ...
  xx = (pkt_ptr->pos[0] - (cell[0].pos_init[0]*t_current/tmin)) / (wid_init*t_current/tmin);
  yy = (pkt_ptr->pos[1] - (cell[0].pos_init[1]*t_current/tmin)) / (wid_init*t_current/tmin);
  zz = (pkt_ptr->pos[2] - (cell[0].pos_init[2]*t_current/tmin)) / (wid_init*t_current/tmin);
