...
double
boundary_cross (pkt_ptr, tstart, snext)
     PKT *pkt_ptr;
     double tstart;
     int *snext;
     //   double *closest;
{
  double x0, y0, z0;
  double vx, vy, vz;
  double cellxmin, cellxmax, cellymin, cellymax, cellzmin, cellzmax;
  double tx_plus, tx_minus, ty_plus, ty_minus, tz_plus, tz_minus;
  double time, distance;
  int choice, not_allowed;
  //double close, close_try;

  /** There are six possiblt boundary crossings. Each of the three
  ...
  vx = pkt_ptr->dir[0] * CLIGHT_PROP;
  vy = pkt_ptr->dir[1] * CLIGHT_PROP;
  ...
  if ((((x0 * tmin) - (tstart * cellxmax)) > (-10. * tmin))
      && (not_allowed != NEG_X))
    {
      printout
        ("[warning] outside x boundary of cell %d. x0 %g, cellxmin %g, cellxmax %g. Abort?\n",
         pkt_ptr->where, x0 * tmin / tstart, cellxmin, cellxmax);
      printout
        ("[warning] outside x boundary of cell %d. y0 %g, cellymin %g, cellymax %g. Abort?\n",
         pkt_ptr->where, y0 * tmin / tstart, cellymin, cellymax);
      printout
        ("[warning] outside x boundary of cell %d. z0 %g, cellzmin %g, cellzmax %g. Abort?\n",
      ...
      printout ("[warning] dir [%g, %g, %g]\n", pkt_ptr->dir[0],
                pkt_ptr->dir[1], pkt_ptr->dir[2]);
      if (((pkt_ptr->dir[0] * CLIGHT_PROP) - (x0 / tstart)) > 0)
        {
  ...
  if ((((x0 * tmin) - (tstart * cellxmin)) < (10. * tmin))
      && (not_allowed != POS_X))
    {
      printout
        ("[warning] outside x boundary of cell %d. x0 %g, cellxmin %g, cellxmax %g. Abort?\n",
  ...
  if ((((y0 * tmin) - (tstart * cellymax)) > (-10. * tmin))
      && (not_allowed != NEG_Y))
    {
      printout
        ("[warning] outside y boundary of cell %d. x0 %g, cellxmin %g, cellxmax %g. Abort?\n",
         pkt_ptr->where, x0 * tmin / tstart, cellxmin, cellxmax);
      printout
        ("[warning] outside y boundary of cell %d. y0 %g, cellymin %g, cellymax %g. Abort?\n",
         pkt_ptr->where, y0 * tmin / tstart, cellymin, cellymax);
      printout
        ("[warning] outside y boundary of cell %d. z0 %g, cellzmin %g, cellzmax %g. Abort?\n",
         pkt_ptr->where, z0 * tmin / tstart, cellzmin, cellzmax);
      printout ("[warning] pkt_ptr->number %d\n", pkt_ptr->number);
      printout ("[warning] delta %g\n", cellymax - (y0 * tmin / tstart));
      printout ("[warning] dir [%g, %g, %g]\n", pkt_ptr->dir[0],
                pkt_ptr->dir[1], pkt_ptr->dir[2]);
      if (((pkt_ptr->dir[1] * CLIGHT_PROP) - (y0 / tstart)) > 0)
        {
  ...
  if ((((y0 * tmin) - (tstart * cellymin)) < (10. * tmin))
      && (not_allowed != POS_Y))
    {
      printout
        ("[warning] outside y boundary of cell %d. x0 %g, cellxmin %g, cellxmax %g. Abort?\n",
  ...
  if ((((z0 * tmin) - (tstart * cellzmax)) > (-10. * tmin))
      && (not_allowed != NEG_Z))
    {
      printout
        ("[warning] outside z boundary of cell %d. x0 %g, cellxmin %g, cellxmax %g. Abort?\n",
         pkt_ptr->where, x0 * tmin / tstart, cellxmin, cellxmax);
      printout
        ("[warning] outside z boundary of cell %d. y0 %g, cellymin %g, cellymax %g. Abort?\n",
         pkt_ptr->where, y0 * tmin / tstart, cellymin, cellymax);
      printout
        ("[warning] outside z boundary of cell %d. z0 %g, cellzmin %g, cellzmax %g. Abort?\n",
         pkt_ptr->where, z0 * tmin / tstart, cellzmin, cellzmax);
      printout ("[warning] pkt_ptr->number %d\n", pkt_ptr->number);
      printout ("[warning] delta %g\n", cellzmax - (z0 * tmin / tstart));
      printout ("[warning] dir [%g, %g, %g]\n", pkt_ptr->dir[0],
                pkt_ptr->dir[1], pkt_ptr->dir[2]);
      if (((pkt_ptr->dir[2] * CLIGHT_PROP) - (z0 / tstart)) > 0)
        {
  ...
  if ((((z0 * tmin) - (tstart * cellzmin)) < (10. * tmin))
      && (not_allowed != POS_Z))
    {
      printout
        ("[warning] outside z boundary of cell %d. x0 %g, cellxmin %g, cellxmax %g. Abort?\n",
  ...
  if (debuglevel == 2)
    {
      ...
      printout ("delta1z %g delta2z %g\n", (z0 * tmin / tstart) - cellzmin,
                cellzmax - (z0 * tmin / tstart));
      printout ("dir [%g, %g, %g]\n", pkt_ptr->dir[0], pkt_ptr->dir[1],
                pkt_ptr->dir[2]);
    }
#endif

  if (pkt_ptr->number == 480)
    {
      printout ("");
    }

  tx_plus = ((x0 - (vx * tstart)) / (cellxmax - (vx * tmin)) * tmin) - tstart;
  tx_minus =
    ((x0 - (vx * tstart)) / (cellxmin - (vx * tmin)) * tmin) - tstart;

  ty_plus = ((y0 - (vy * tstart)) / (cellymax - (vy * tmin)) * tmin) - tstart;
  ty_minus =
    ((y0 - (vy * tstart)) / (cellymin - (vy * tmin)) * tmin) - tstart;

  tz_plus = ((z0 - (vz * tstart)) / (cellzmax - (vz * tmin)) * tmin) - tstart;
  tz_minus =
    ((z0 - (vz * tstart)) / (cellzmin - (vz * tmin)) * tmin) - tstart;
  ...
  if (choice == 0)
    {
      printout
        ("Something wrong in boundary crossing - didn't find anything.\n");
...
//This is all for debugging 
  if (pkt_ptr->number == 480)
    {
      int xCellIndex = (int) ((x0 + 0.5 * nxgrid * wid_init) / wid_init);
      int yCellIndex = (int) ((y0 + 0.5 * nxgrid * wid_init) / wid_init);
      int zCellIndex = (int) ((z0 + 0.5 * nxgrid * wid_init) / wid_init);
      int cellId =
        zCellIndex * nygrid * nxgrid + yCellIndex * nxgrid + xCellIndex;

      printout ("[TMP DEBUG] Boundary:pkt_ptr->number %d\n", pkt_ptr->number);
      printout ("[TMP DEBUG] Boundary:pkt_ptr->type %d\n", pkt_ptr->type);
      printout ("[TMP DEBUG] Boundary:Cell                 %d\n",
                pkt_ptr->where);
      printout ("[TMP DEBUG] Boundary:Computed cell number %d\n", cellId);
      printout
        ("[TMP DEBUG] Boundary:Computed cell index xi %d yi %d zi %d \n",
         xCellIndex, yCellIndex, zCellIndex);
      printout ("[TMP DEBUG] Boundary:x0 %g y0 %g z0 %g \n", x0, y0, z0);
      printout ("[TMP DEBUG] Boundary:dir [%g, %g, %g]\n", pkt_ptr->dir[0],
                pkt_ptr->dir[1], pkt_ptr->dir[2]);
      printout ("[TMP DEBUG] Boundary:vx %g vy %g vy %g \n", vx, vy, vz);
      printout ("[TMP DEBUG] Boundary:delta1x %g delta2x %g\n",
                (x0 * tmin / tstart) - cellxmin,
                cellxmax - (x0 * tmin / tstart));
      printout ("[TMP DEBUG] Boundary:delta1y %g delta2y %g\n",
                (y0 * tmin / tstart) - cellymin,
                cellymax - (y0 * tmin / tstart));
      printout ("[TMP DEBUG] Boundary:delta1z %g delta2z %g\n",
                (z0 * tmin / tstart) - cellzmin,
                cellzmax - (z0 * tmin / tstart));

      printout ("[TMP DEBUG] Boundary:tx_plus %g tx_minus %g \n", tx_plus,
                tx_minus);
      printout ("[TMP DEBUG] Boundary:ty_plus %g ty_minus %g \n", ty_plus,
                ty_minus);
      printout ("[TMP DEBUG] Boundary:tz_plus %g tz_minus %g \n", tz_plus,
                tz_minus);
      printout ("[TMP DEBUG] Boundary:tstart %g\n", tstart);
      printout ("[TMP DEBUG] Boundary:time %g \n", time);
    }

  /** Now we know what happens. The distance to crossing is....*/
  // The last part of the therm add a small  distance to the sdist to avoid placing a pkt on the edge of the cell.
  distance = CLIGHT_PROP * time;
  //*closest = close;
     ...
     int *end_packet;
{
  ...
  if (snext == -99)
    ...
    /** Just need to update "where".*/
      ...
      //Here, we change the Methode to set the cell information. In the original artis code,  this was done by using the cell information from boundary_change. See comment. In the new version,  we compute the cell id of the packet by using his position stored in pkt_ptr.
      //   pkt_ptr->where = snext; // This is the old cell_id treatment.
//This is the block of the new treatment.
      {
        int cellId;
        int grid[3] = { nxgrid,nygrid,nzgrid};
        cellId = get_cell_id(pkt_ptr->pos, wid_init *(t_current / tmin), grid); // The factor  (t_current / tmin) is for the homologous expansion of the grid.

        if (cellId == snext)
        {
          pkt_ptr->where = cellId;
        }
        else
        {
          printout("The snext (%i) is not equal to the computed cell (%i)",snext, cellId);
          pkt_ptr->where = cellId;


        }
        //pkt_ptr->where = get_cell_id(pkt_ptr->pos, wid_init, grid);
      }
          ...
          if (pkt_ptr->type == TYPE_RPKT)
            {
              ...
              if (debuglevel == 2)
                printout
                  ("[debug] calculate_kappa_rpkt after cell crossing\n");
#endif
              /// This only needs to be done for non-grey cells
              if (modelgrid[mgi].thick != 1)
                {
...
int
locate (pkt_ptr, t_current)
     PKT *pkt_ptr;
     double t_current;
{
  // This is the modified version of locate. Here we use get_cell_id to compute the cellID. The homologous expansion is treaded by scaling the cell width with (t_current / tmin).
  
  double cCellWid = wid_init * (t_current / tmin); // The current width of the cells.
  int grid[3] = { nxgrid,nygrid,nzgrid};
  return get_cell_id ( pkt_ptr->pos, cCellWid, grid);
  //int xx, yy, zz, ans;

  ///* Cheap and nasty version for now - assume a uniform grid. */
  //xx =
    //(pkt_ptr->pos[0] -
     //(cell[0].pos_init[0] * t_current / tmin)) / (wid_init * t_current /
                                                  //tmin);
  //yy =
    //(pkt_ptr->pos[1] -
     //(cell[0].pos_init[1] * t_current / tmin)) / (wid_init * t_current /
                                                  //tmin);
  //zz =
    //(pkt_ptr->pos[2] -
     //(cell[0].pos_init[2] * t_current / tmin)) / (wid_init * t_current /
                                                  //tmin);

  //ans = xx + (nxgrid * yy) + (nxgrid * nygrid * zz);
  //return (ans);
}


int
get_cell_id (double x[3], double cellWid, int grid[3])
{
  //This function computes the CellId to the given position. All computations are done with integer values.
  //x[3] is the position vector, where [0] [1] [2] are  the x y z components. 
  //cellWid defines the width of the cell.
  //grid[3] is the dimension of the grid. The order of the component is the same as for the position vector.
  int index[3];
  int i;
  int xExp;
  double xMantissa;
  ...
  wMantissa = fabs (frexp (cellWid, &wExp));	//Returns the normalised Mantissa.

  for (i = 0; i < 3; i++)
    {
// Loop over all vector components.
      xMantissa = (frexp (x[i], &xExp));
      minE = wExp < xExp ? wExp : xExp;	// Obtain the maximum common exponent.
      index[i] = 
	(ldexp (xMantissa, xExp - minE + 1) +
	 grid[i] * ldexp (wMantissa, wExp - minE)) / ldexp (wMantissa,
							    wExp - minE + 1);
    }

  printf ("new xi %d yi %d zi %d \n", index[0], index[1], index[2]);
  return index[0] + index[1] * grid[0] + index[2] * grid[1] * grid[0];
}
