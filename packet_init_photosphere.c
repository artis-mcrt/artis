...
#endif
int
packet_init_photosphere (int middle_iteration, int my_rank)
{
...
int
setup_packets_photosphere (int pktnumberoffset)
/// Subroutine that initialises the packets if we start a new simulation.
{
  ...
  double doppler_shift (const size_t n, double *directionVec,
			double *velocityVec);
  double get_cell_velocity_3 (double *positionVecPrt,
			      double *velocityVecPrt, double time);
  ...
  for (n = 0; n < npkts; n++)
    {
      ...
      ynEmittPosition = sin (thetaEmittPosition) * sin (phiEmittPosition);
      ...
      while (1)
	{
	  ...
	  thetaEmittDirectionLF =
	    acos (1 - 2 * gsl_rng_uniform (rng)) - M_PI / 2.;
	  phiEmittDirectionLF = M_PI * gsl_rng_uniform (rng) - M_PI / 2.;

	  //Transforming the photon direction from the local frame(LF) to the global frame(GF)
	  xnEmittDirection =
	    sin (thetaEmittDirectionLF) *
	    cos (phiEmittDirectionLF) * cos (thetaEmittPosition) *
	    cos (phiEmittPosition)
	    -
	    sin (thetaEmittDirectionLF) *
	    sin (phiEmittDirectionLF) * cos (thetaEmittPosition)
	    +
	    cos (thetaEmittDirectionLF) * cos (phiEmittPosition) *
	    sin (thetaEmittPosition);

	  ynEmittDirection =
	    sin (thetaEmittDirectionLF) *
	    cos (phiEmittDirectionLF) * cos (thetaEmittPosition) *
	    sin (phiEmittPosition)
	    +
	    sin (thetaEmittDirectionLF) *
	    sin (phiEmittDirectionLF) * cos (phiEmittPosition)
	    +
	    cos (thetaEmittDirectionLF) * sin (phiEmittPosition) *
	    sin (thetaEmittPosition);

	  znEmittDirection =
	    sin (thetaEmittDirectionLF) *
	    cos (phiEmittDirectionLF) * sin (thetaEmittPosition)
	    + cos (thetaEmittDirectionLF) * cos (thetaEmittPosition);

	  //check if direction is valid.
	  norm =
	    sqrt (gsl_pow_2
		  (xnEmittDirection +
		   xnEmittPosition * rPhotosphere) +
		  gsl_pow_2 (ynEmittDirection +
			     ynEmittPosition * rPhotosphere) +
		  gsl_pow_2 (znEmittDirection +
			     znEmittPosition * rPhotosphere));

	  if (norm >= rPhotosphere)
	    {
	      break;
	    }
	}
...
#define RAND_NU_PLANCK(v,T) ((c1 * gsl_pow_3((v)))/(gsl_sf_exp(c2*((v)/(T))) - 1 ))
  ...
  const double c1 =
    2 * GSL_CONST_CGS_PLANCKS_CONSTANT_H /
    (gsl_pow_2 (GSL_CONST_CGS_SPEED_OF_LIGHT));
  const double c2 =
    GSL_CONST_CGS_PLANCKS_CONSTANT_H / GSL_CONST_CGS_BOLTZMANN;
  ...
  if (((nuPeak) > nu_max_r) && ((nuPeak) < nu_min_r))
    {
      printf
	("The Planck function peaks(%g < %g < %g) outside the configured frequency range. Abort.\n",
...
double
get_cell_velocity_3 (double *positionVecPrt, double *velocityVecPrt,
		     double time)
{
