...
static auto calculate_cooling_rates_ion(const int modelgridindex, const int element, const int ion,
                                        const int indexionstart, double *C_ff, double *C_fb, double *C_exc,
...
// oldcoolingsum is the sum of lower ion (of same element or all ions of lower elements) cooling contributions
{
  ...
  if (ioncharge > 0) {
    ...
    if constexpr (update_cooling_contrib_list) {
      ...
      // %d\n",contrib,oldcoolingsum,C,element,ion,-99,globals::cellcache[cellcacheslotid].coolinglist[i].type,i,low);
      i++;
    } else {
      *C_ff += C_ff_ion;
    }
...
// optionally store components (ff, bf, collisional) in heatingcoolingrates struct
{
  ...
  for (int element = 0; element < get_nelements(); element++) {
    ...
    for (int ion = 0; ion < nions; ion++) {
      const double C_ion = calculate_cooling_rates_ion<false>(modelgridindex, element, ion, -1, &C_ff_all, &C_fb_all,
...
/// handle a k-packet (kinetic energy of the free electrons)
{
  ...
  if (globals::cellcache[cellcacheslotid].cooling_contrib[ihigh] < 0.) {
    ...
    const double C_ion =
        calculate_cooling_rates_ion<true>(modelgridindex, element, ion, ilow, nullptr, nullptr, nullptr, nullptr);
