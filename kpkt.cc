...
DEVICE_FUNC void do_kpkt_blackbody(Packet& pkt) {
  ...
  if (RPKT_BOUNDBOUND_THERMALISATION_PROBABILITY.has_value() && grid::thick_allcells[nonemptymgi] != 1) {
    ...
    pkt.nu_cmf = sample_planck_montecarlo(grid::get_Te(nonemptymgi), pkt.rngstate());
  }
...
DEVICE_FUNC void do_kpkt(Packet& pkt, const double t2, const int nts) {
  ...
  const double rndcool_ion = rng_uniform(pkt.rngstate()) * ion_cooling_contribs_thiscell.back();
  ...
  const double rndcool_ion_process = rng_uniform(pkt.rngstate()) * C_ion_procsum;
  ...
  if (rndcoolingtype == CoolingType::FREEFREE) {
    ...
    pkt.nu_cmf = -KB * T_e / H * std::log(static_cast<double>(rng_uniform_pos(pkt.rngstate())));
    ...
    pkt.nu_cmf = select_continuum_nu(element, lowerion, lowerlevel, upper, T_e, pkt.rngstate());
