...
paths="input.txt.tmp input-newrun.txt.tmp gridsave*.tmp packets*.tmp vspecpol*.tmp vpackets*.tmp vpkt_grid*.tmp *.out *.out.* out.txt output_+([0-9])-+([0-9]).txt?(.zst|.gz|.xz) exspec*.txt* machine.file.* core.* *.slurm job_from_ts+([0-9]) packets vspecpol vpackets vpkt_grid speclc_angle_res bflist.dat ratecoeff.dat line_list.txt logfiles.tar*"
