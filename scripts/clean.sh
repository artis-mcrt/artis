...
paths="gridsave*.tmp packets*.tmp vspecpol*.tmp vpackets*.tmp *.out *.out.* out.txt output_+([0-9])-+([0-9]).txt?(.zst|.gz|.xz) exspec*.txt* machine.file.* core.* *.slurm +([0-9]).job packets vspecpol vpackets speclc_angle_res bflist.dat ratecoeff.dat line_list.txt logfiles.tar*"
