...
def get_mabins() -> int:
    ...
    exspec_h = (Path(__file__).resolve().parent.parent / "exspec.h").read_text(
        encoding="utf-8"
    )
    bincounts = {
        name: int(value)
        for name, value in re.findall(
            r"constexpr int (NPHIBINS|NCOSTHETABINS) = (\d+);", exspec_h
        )
    }
...
def main() -> None:
    ...
    for basefolder in [Path(), Path("speclc_angle_res")]:
        ...
        for fileprefix in ["light_curve", "spec", "specpol"]:
            ...
            input_files = [
                resfilepath(basefolder, fileprefix, abin) for abin in range(mabins)
            ]
            if all(
                input_file.is_file() and input_file.stat().st_size > 0
                ...
                with outfile.open("wt", encoding="utf-8") as fout:
                    for input_file in input_files:
                        fout.writelines(
                            input_file.open("rt", encoding="utf-8").readlines()
                        )
            else:
                print(
                    f"Some {fileprefix}_res_??.out files are missing or empty in {basefolder}"
                )
