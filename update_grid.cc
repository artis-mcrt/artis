...
void write_to_estimators_file(std::ostream& estimators_file, const int nonemptymgi, const int timestep, const int titer,
  ...
  if (write_estim_duration >= 1) {
    logprintlnfmt("writing estimators for timestep {} cell {} took {} seconds", timestep, mgi, write_estim_duration);
  }
}
...
void solve_Te_nltepops(const int nonemptymgi, const int nts, const int nts_prev,
  ...
  logprintfmt("calculate_bfheatingcoeffs for timestep {} cell {}...", nts, mgi);
  ...
  logprintlnfmt("took {} seconds", std::time(nullptr) - sys_time_start_calculate_bfheatingcoeffs);
  ...
  for (int nlte_iter = 0; nlte_iter <= NLTEITER; nlte_iter++) {
    ...
    if (nlte_iter == NLTEITER) {
      logprintlnfmt("WARNING: NLTE solver failed to converge after {} iterations. Keeping solution from last iteration",
                    nlte_iter + 1);
    }
  }
}
...
void update_grid_cell(const int nonemptymgi, const int nts, const int nts_prev, const int titer, const double tratmid,
  ...
  logprintlnfmt("update_grid_cell: working on mgi {} before timestep {} titeration {}...", mgi, nts, titer);
  ...
  if (nts == globals::timestep_initial && titer == 0) {
    ...
    logprintlnfmt("mgi {} modelgrid.thick: {} (during grid update)", mgi, grid::modelgrid[nonemptymgi].thick);
    ...
    logprintlnfmt("Temperature/NLTE solution for cell {} timestep {} took {} seconds", mgi, nts,
  ...
  logprintlnfmt(
      "modelgridindex {}, compton optical depth (/propgridcell) {:g}, grey optical depth (/propgridcell) {:g}", mgi,
      compton_optical_depth_across_cell, grey_optical_depth_across_cell);
  logprintlnfmt("radial_pos {:g}, distance_to_obs {:g}, tau_dist {:g}", radial_pos, dist_to_obs, grey_optical_depth);
  ...
  if ((grey_optical_depth >= globals::cell_is_optically_thick) && (nts < globals::num_grey_timesteps)) {
    logprintlnfmt("timestep {} cell {} is treated in grey approximation (chi_grey {:g} [cm2/g], tau {:g} >= {:g})", nts,
  ...
  if (grid::modelgrid[nonemptymgi].thick == 1) {
    ...
    logprintfmt("calculating cooling_rates for timestep {} cell {}...", nts, mgi);
    ...
    logprintlnfmt("took {} seconds", std::time(nullptr) - sys_time_start_calc_kpkt_rates);
  }
  ...
  if (update_grid_cell_seconds > 0) {
    logprintlnfmt("update_grid_cell for cell {} timestep {} took {} seconds", mgi, nts, update_grid_cell_seconds);
  }
}
...
void update_grid(std::ostream& estimators_file, const int nts, const int nts_prev, const int titer,
  ...
  logprintlnfmt("");
  logprintlnfmt("timestep {}: time before update grid {} (tstart + {}) simtime ts_mid {:g} days", nts,
  ...
  if (globals::opacity_case == 3) {
    logprintlnfmt("update_grid: rho_crit = {:g}", globals::rho_crit);
  }
...
#pragma omp parallel for schedule(dynamic)
  ...
  logprintlnfmt("max_path_step {:g}", globals::max_path_step);

  const auto time_update_grid_end_thisrank = std::time(nullptr);
  logprintlnfmt("finished update grid on this rank at time {}", time_update_grid_end_thisrank);

  MPI_Barrier(MPI_COMM_WORLD);
  logprintlnfmt("timestep {}: time after update grid for all processes {} (rank {} took {}s, waited {}s, total {}s)",
