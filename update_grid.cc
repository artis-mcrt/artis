...
// nts is the timestep number
{
  ...
  for (int nlte_iter = 0; nlte_iter <= NLTEITER; nlte_iter++) {
    ...
    for (int element = 0; element < get_nelements(); element++) {
      ...
      if ((USE_LUT_PHOTOION && (nlte_iter > 0)) || !elem_has_nlte_levels(element)) {
        // recalculate the Gammas using the current population estimates const int nions = get_nions(element);
        const int nions = get_nions(element);
        for (int ion = 0; ion < nions - 1; ion++) {
          globals::gammaestimator[get_ionestimindex_nonemptymgi(nonemptymgi, element, ion)] =
