...
static __managed__ struct collionrow *colliondata = NULL;
...
static __managed__ double E_init_ev = 0;         // the energy injection rate density (and mean energy of injected electrons if source integral is one) in eV
  ...
  static __managed__ double delta_log_e = 0.;
#else
...
// should be in erg / s / cm^3
{
  ...
  // }
  assert(deposition_rate_density_timestep[modelgridindex] == nts_global);
...
void nt_close_file(void)
{
  ...
  gsl_vector_free_managed(envec);
  gsl_vector_free_managed(sourcevec);
  for (int modelgridindex = 0; modelgridindex < npts_model; modelgridindex++)
  {
    if (get_numassociatedcells(modelgridindex) > 0)
    {
...
__host__ __device__
inline static int get_energyindex_ev_lteq(const double energy_ev)
...
__host__ __device__
static inline int get_energyindex_ev_gteq(const double energy_ev)
// finds the highest energy point <= energy_ev
{
  #if (SF_USE_LOG_E_INCREMENT)
...
// select based on stored frac_deposition for each ion
{
  ...
  for (int allionindex = 0; allionindex < includedions; allionindex++)
  {
...
#if CUDA_ENABLED
__global__ static void kernel_sfmatrix_add_excitation_transition(gsl_matrix *sfmatrix, float *arr_epsilon_trans, int nuptrans_selected, int *uptrans_lineindicies, double statweight_lower, double nnlevel_lower)
{
  ...
  if (i < SFPTS)
  {
    ...
    if (t < nuptrans_selected)
    {
      ...
      if (xsstartindex >= 0)
      {
        ...
        {
          double xs_deltae_j = xs_excitation(lineindex, arr_epsilon_trans[t], envec->data[j] * EV);
          #if (SF_USE_LOG_E_INCREMENT)
            xs_deltae_j *= delta_envec->data[j];
          #else
            xs_deltae_j *= DELTA_E;
          #endif
...
static void sfmatrix_add_excitation_transitions_gpu(gsl_matrix *sfmatrix, float *arr_epsilon_trans, int *uptrans_lineindicies, int nuptrans_selected, double statweight_lower, double nnlevel_lower)
{
  checkCudaErrors(cudaDeviceSynchronize());

  dim3 threadsPerBlock(64, 1, 1);
  dim3 numBlocks(ceil(SFPTS / 64.), nuptrans_selected, 1);
...
static void sfmatrix_add_excitation_transition(gsl_matrix *sfmatrix, int lineindex, double statweight_lower, double epsilon_trans, double nnlevel_lower)
{
  ...
  if (xsstartindex >= 0)
  {
    ...
    for (int i = 0; i < SFPTS; i++)
    {
      ...
      for (int j = startindex; j < stopindex; j++)
      {
        *gsl_matrix_ptr(sfmatrix, i, j) += nnlevel_lower * vec_xs_excitation_deltae->data[j];
      }
      ...
      *gsl_matrix_ptr(sfmatrix, i, stopindex) += nnlevel_lower * vec_xs_excitation_deltae->data[stopindex] * delta_en_actual / delta_en;
    }
  }
...
static void sfmatrix_add_excitation(gsl_matrix *sfmatrix, const int modelgridindex, const int element, const int ion, double *E_0)
{
  ...
  for (int lower = 0; lower < nlevels; lower++)
  {
    ...
    int *arr_lineindicies;
    cudaMallocManaged(&arr_lineindicies, nuptrans * sizeof(int));
    int nuptrans_selected = 0;
    #endif
    ...
    for (int t = 0; t < nuptrans; t++)
    {
      ...
      #if (CUDA_ENABLED && USECUDA_NONTHERMAL_EXCITATION)
      arr_epsilon_trans[nuptrans_selected] = epsilon_trans;
      arr_lineindicies[nuptrans_selected] = lineindex;
      nuptrans_selected++;
      #endif
    ...
    #if (CUDA_ENABLED && USECUDA_NONTHERMAL_EXCITATION)
    sfmatrix_add_excitation_transitions_gpu(sfmatrix, arr_epsilon_trans, arr_lineindicies, nuptrans_selected, statweight_lower, nnlevel);
    cudaFree(arr_epsilon_trans);
    cudaFree(arr_lineindicies);
    #endif
  }
}
...
static void sfmatrix_solve(const gsl_matrix *sfmatrix, const gsl_vector *rhsvec, gsl_vector *yvec)
{
  ...
  // gsl_matrix *sfmatrix_LU = gsl_matrix_alloc_managed(SFPTS, SFPTS);
