...
sn3d writes the per-job output files (the rank log files and the estimators, nlte, radfield, and macroatom files) into a job folder. sn3d names the folder from the job index: `00000000.job` for a new simulation, then `00000001.job` for the first resumed job, and so on. The last line of input.txt stores the index of the next job, and sn3d updates it with each set of restart files. sn3d adds this line if it is absent. Rank 0 writes the job index and the job folder to the standard output, so the log of a Slurm job names its folder. A new simulation first removes the files of the previous simulation: the output files, the restart files, and the `*.job` and `*.slurm` folders. It removes the same files as `scripts/clean.sh`, except `slurm-*.out`, `machine.file.*`, and `core.*`, which can belong to the current job. sn3d writes the shared run-level files, including the restart files, to the simulation folder. It also keeps an `output_0-0.txt` symlink there that points to the rank-0 log of the current job.
...
## Bundled scripts
- clean.sh: Remove all output files while keeping input files and resetting the simulation to the beginning. The script also removes the *.job folders of sn3d and the *.slurm folders of older versions.
