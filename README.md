...
sn3d writes the per-job output files (the rank log files and the estimators, nlte, radfield, and macroatom files) into a job folder. Without the `-o` option, sn3d names the folder from the job index: `00000000.job` for a new simulation, then `00000001.job` for the first resumed job, and so on. The last line of input.txt stores the index of the next job, and sn3d updates it with each set of restart files. sn3d adds this line if it is absent. A new simulation first removes the files of the previous simulation: the output files, the restart files, and the `*.job` and `*.slurm` folders. It removes the same files as `scripts/clean.sh`, except `slurm-*.out`, `machine.file.*`, and `core.*`, which can belong to the current job. Run sn3d with `-o JOBFOLDER` (e.g. `./sn3d -o job0`) to give a different name. The cluster job scripts do this with a folder named after the SLURM job id. sn3d writes the shared run-level files, including the restart files, to the simulation folder. It also keeps an `output_0-0.txt` symlink there that points to the rank-0 log of the current job.
