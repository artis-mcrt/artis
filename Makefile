...
ifeq ($(STDPAR),ON)
  ...
  ifeq ($(COMPILER_NAME),NVHPC)
		...
		CXXFLAGS += --hipstdpar --hipstdpar-interpose-alloc --hipstdpar-path=/opt/rocm-7.2.0/include/thrust/system/hip/hipstdpar/ --hipstdpar-thrust-path=/opt/rocm-7.2.0/include/thrust
		ifeq ($(GPU),ON)
			# MI300
