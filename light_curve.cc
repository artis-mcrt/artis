...
void write_light_curve(const std::string &lc_filename, const int current_abin,
  ...
  for (int nts = 0; nts < numtimesteps; nts++) {
    assert_always(snprintf(linebuffer, maxlen, "%g %g %g", globals::timestep[nts].mid / DAY,
  ...
  if (current_abin == -1) {
    ...
    for (int m = 0; m < numtimesteps; m++) {
      assert_always(snprintf(linebuffer, maxlen, "%g %g %g", globals::timestep[m].mid / DAY,
                             (globals::timestep[m].gamma_dep / LSUN / globals::timestep[m].width),
                             (globals::timestep[m].cmf_lum / globals::timestep[m].width / LSUN)) < maxlen);
      lc_file << linebuffer << '\n';
    }
  }
}
...
// add a packet to the outgoing light-curve.
{
  if (current_abin == -1) {
    ...
    if (arrive_time > globals::tmin && arrive_time < globals::tmax) {
      ...
      safeadd(light_curve_lum[nt], pkt_ptr->e_rf / globals::timestep[nt].width / globals::nprocs_exspec);
    }
    ...
    if (arrive_time_cmf > globals::tmin && arrive_time_cmf < globals::tmax) {
      ...
      safeadd(light_curve_lumcmf[nt], pkt_ptr->e_cmf / globals::timestep[nt].width / globals::nprocs_exspec /
  ...
  if (get_escapedirectionbin(pkt_ptr->dir, globals::syn_dir) == current_abin) {
    ...
    if (t_arrive > globals::tmin && t_arrive < globals::tmax) {
      ...
      safeadd(light_curve_lum[nt], pkt_ptr->e_rf / globals::timestep[nt].width * MABINS / globals::nprocs_exspec);
    }
  }
}