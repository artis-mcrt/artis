...
/// Material for handling activated macro atoms.
{
  ...
  if (mastate[tid].level == 0) 
  {
    do_internalupsame = 0;
    do_internaluphigher= 0;
  }
  else
  {
    do_internalupsame = 1;
    do_internaluphigher = 1;
  }
  do_internaldownlower = 1;
  //debuglevel = 2;
  while (end_packet == 0)
  {
    ion = mastate[tid].ion;
    ...
    else
    {
      ...
      {
        /// so far this happens only if level = 0 (checked by rad_recomb/col_recomb)
        /// an explicit check here could speed up things
        if (level == 0)
        {
          if (do_internaldownlower)
          {
      ...
      if (do_internaluphigher  == 1)
      {
      if (ion < get_nions(element)-1 && level < ionisinglevels)  //&& get_ionstage(element,ion) < get_element(element)+1)
      {
        /// so far only ionisations to the ground level are treated (checked by photoionozation/col_ionization)
        /// an explicit check here could speed up things
        //for (upper=0; upper < get_nlevels(element,ion+1); upper++)
        //{
        upper = 0;
      ...
      //internal_up_higher = 0.;  ///DEBUG ONLY
      }
    ...
    else if (zrand*total_transitions < rad_deexc + col_deexc + internal_down_same + rad_recomb)
    {
      ...
      for (lower=0; lower < nlevels; lower++)
      {
        ...
        #ifdef DEBUG_ON
          if (debuglevel == 2) printout("[debug] do_ma:   R %g, deltae %g\n",R,(epsilon(element,ion,level)-epsilon(element,ion-1,lower)));
          if (debuglevel == 2) printout("[debug] do_ma:   rate to level %d of ion %d = %g\n",lower,ion-1,rate);
          if (debuglevel == 2) printout("[debug] do_ma:   zrand*rad_recomb = %g\n",zrand*rad_recomb);
        #endif
      ...
      gsl_integration_qag(&F_alpha_sp, nu_threshold, 10*nu_threshold, 0, intaccuracy, 1000, 6, wsp, &total_alpha_sp, &error);
      ...
      for (i= 0; i < 100; i++)
      {
        ...
        if (i > 0)
        {
          ...
          gsl_integration_qag(&F_alpha_sp, nu_lower, 10*nu_threshold, 0, intaccuracy, 1000, 6, wsp, &alpha_sp, &error);
          //alpha_sp *= FOURPI * sf;
...
/*      if (element == 6) 
    ...
    #endif
    {
      ...
      /// Randomly select the occuring transition
      /// As long as we allow only ionisation to the groundlevels it is just
      upper = 0;
      /*
      zrand = gsl_rng_uniform(rng);
      rate = 0.;
      ...
      {
        epsilon_trans = epsilon(element,ion+1,upper) - epsilon_current;
        R = photoionization(pkt_ptr,upper,epsilon_trans);
        C = col_ionization(pkt_ptr,upper,epsilon_trans);
...
/// collisional ionization rate: paperII 3.5.1
{
  ...
  if (upper == 0)
  {
    ...
    if (ionstage == 1) g = 0.1;
    else if (ionstage == 2) g = 0.2;
    else g = 0.3;
    C = n_l*nne  * 1.55e13 * pow(T_e,-0.5) * g * photoionization_crosssection(nu_lower, nu_lower) * exp(-fac1)/fac1; ///photoionization at the edge
...
/// collisional recombination rate: paperII 3.5.1
{
  ...
  ///as long as ionization only happens to the ground state of the next ionisation stage
  ///recombination is only allowed from the ground state to conserve level populations
  if (upper == 0)
  {
    ///Seaton approximation: Mihalas (1978), eq.5-79, p.134
    ///select gaunt factor according to ionic charge
    ...
    #endif
    if (ionstage-1 == 1) g = 0.1;
    else if (ionstage-1 == 2) g = 0.2;
    else g = 0.3;
    ...
    sigma_bf = photoionization_crosssection(nu_lower, nu_lower);
    #ifdef DEBUG_ON
      if (debuglevel == 777) printout("sigma_bf %g\n",sigma_bf);
    #endif
    mastate[tid].ion = ion;
    mastate[tid].level = upper;    ///restore the old values of pkt_ptr
    C = n_u*nne*nne * get_sahafact(element,ion-1,lower,T_e,epsilon_trans) * 1.55e13 * pow(T_e,-0.5) * g * sigma_bf * exp(-fac1)/fac1;
