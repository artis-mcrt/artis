...
// update the phixslist
{
  ...
  *kappa_fb = 0.;
  ...
  const double boltzmannfactor = exp(-HOVERKB * nu / T_e);;
  for (int i = 0; i < nbfcontinua; i++)
  {
    ...
    if (nu >= nu_edge)
    {
      ...
      if (nu <= nu_max_phixs && (get_abundance(modelgridindex,element) > 0) && (DETAILED_BF_ESTIMATORS_ON || (level < 100) || (ionstagepop(modelgridindex, element, ion) / nnetot > 1.e-6)))
      {
        ...
        const double stimfactor = departure_ratio * boltzmannfactor;
...
static void calculate_kappa_rpkt_cont(const PKT *restrict const pkt_ptr, const double t_current, const int modelgridindex, const int timestep, struct rpkt_cont_opacity_struct *kappa_continuum)
{
  ...
  if (do_r_lc)
  {
    if (opacity_case == 4)
    {
      ...
      kappa_continuum->bf_inrest = kappa_bf;
      kappa_continuum->fb_inrest = kappa_fb;
  ...
  #ifdef DEBUG_ON
    //if (debuglevel == 2)
    //  printout("[debug]  ____kappa_rpkt____: kappa_cont %g, sigma %g, kappa_ff %g, kappa_bf %g\n",kappa_continuum->total,sigma,kappa_ff,kappa_bf);
  #endif
  kappa_continuum->es = sigma;
  kappa_continuum->ff = kappa_ff;
  kappa_continuum->bf = kappa_bf;
  kappa_continuum->fb = kappa_fb;
  kappa_continuum->ffheating = kappa_ffheating;
  //kappa_continuum->bfheating = kappa_bfheating;

  #ifdef DEBUG_ON
    if (!isfinite(kappa_continuum->total))
    {
      printout("[fatal] calculate_kappa_rpkt_cont: resulted in non-finite kappa_rpkt_cont.total ... abort\n");
      printout("[fatal] es %g, ff %g, bf %g\n",kappa_continuum->es,kappa_continuum->ff,kappa_continuum->bf);
      ...
      if (isfinite(kappa_continuum->es))
      {
        kappa_continuum->ff = 0.;
        kappa_continuum->bf = 0.;
        kappa_continuum->total = kappa_continuum->es;
      }
      else
      {
        abort();
...
void calculate_kappa_vpkt_cont(const PKT *pkt_ptr, const double t_current, struct rpkt_cont_opacity_struct *kappa_continuum)
{
    double sigma;
    ...
    kappa_continuum->total = sigma + kappa_bf + kappa_ff;
    #ifdef DEBUG_ON
...
// return -1 if no transition can be reached
{
  ...
  else
  {
    ...
    if (left > 0)
    {
      if ((linelist[left].nu-nu_cmf)/nu_cmf > 1e-8)
      {
        ...
        //printf("[warning] closest_transition:   nu_cmf %25.16e, linelist[left].nu %25.16e,\n",nu_cmf, linelist[left].nu);
        printout("[warning] closest_transition:   nu_cmf %g, linelist[left].nu %g,\n",nu_cmf, linelist[left].nu);
        printout("[warning] closest_transition:   (linelist[left].nu-nu_cmf)/nu_cmf %g,\n",(linelist[left].nu-nu_cmf)/nu_cmf);
        ...
        if (nu_cmf >= linelist[left+1].nu) printout("[warning] closest_transition:   nu_cmf larger than next line\n");
...
// BE AWARE THAT THIS PROCEDURE SHOULD BE ONLY CALLED FOR NON EMPTY CELLS!!
{
  ...
  while (!endloop)
  {
    ...
    else
    {
      ...
      //printout("nu_cmf %g, opticaldepths in ff %g, es %g\n",pkt_ptr->nu_cmf,kappa_continuum->ff*(abort_dist-dist),kappa_continuum->es*(abort_dist-dist));
      #ifdef DEBUG_ON
        if (debuglevel == 2) printout("[debug] get_event:     tau_rnd %g, tau %g, tau_cont %g\n", tau_rnd, tau, tau_cont);
      #endif
  ...
  struct rpkt_cont_opacity_struct *kappa_continuum, const int modelgridindex)
{
  ...
  const double sigma = kappa_continuum->es;
  const double kappa_ff = kappa_continuum->ff;
  const double kappa_bf = kappa_continuum->bf;
  ...
  else if (zrand * kappa_cont < sigma + kappa_ff + kappa_bf)
  {
    ...
    const double kappa_bf_inrest = kappa_continuum->bf_inrest;
  ...
  else if (zrand * kappa_cont < sigma + kappa_ff + kappa_bf + kappa_continuum->fb)
  {
    ...
    /// Update the bf-opacity for the packets current frequency
    const double kappa_fb_inrest = kappa_continuum.fb_inrest;
...
static void update_estimators(const PKT *pkt_ptr, const double distance, const double t_current, struct rpkt_cont_opacity_struct *kappa_continuum)
  ...
  if (modelgridindex != MMODELGRID)
  {
    ...
    radfield_update_estimators(modelgridindex, distance_e_cmf, nu, pkt_ptr, t_current);
      ...
      ///quick and dirty solution: store info in element=ion=0, and leave the others untouched (i.e. zero)
        ...
        #pragma omp atomic
      #endif
      ffheatingestimator[modelgridindex] += distance_e_cmf * kappa_continuum->ffheating;
    ...
    //cell[pkt_ptr->where].heating_ff += distance_e_cmf * kappa_continuum->ffheating;
    //cell[pkt_ptr->where].heating_bf += distance_e_cmf * kappa_continuum->bfheating;
  }
}
