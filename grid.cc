...
void read_grid_restart_data(const int timestep) {
  ...
  for (int nts = 0; nts < globals::ntimesteps; nts++) {
    ...
    assert_always(fscanf(gridsave_file,
                         "%la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %d ",
                         ...
                         &globals::timesteps[nts].spfission_dep_discrete,
                         &globals::timesteps[nts].eps_spfission_ana_power, &globals::timesteps[nts].qdot_betaminus,
                         &globals::timesteps[nts].qdot_alpha, &globals::timesteps[nts].qdot_spfission,
                         &globals::timesteps[nts].qdot_total, &globals::timesteps[nts].gamma_emission,
                         &globals::timesteps[nts].cmf_lum, &pellet_decays) == 23);
...
void write_grid_restart_data(const int timestep) {
  ...
  for (int nts = 0; nts < globals::ntimesteps; nts++) {
    fprintf(
        gridsave_file, "%la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %la %d ",
        globals::timesteps[nts].gamma_dep, globals::timesteps[nts].gamma_dep_discrete,
        ...
        globals::timesteps[nts].gamma_emission, globals::timesteps[nts].cmf_lum, globals::timesteps[nts].pellet_decays);
  }
...
void init_grid(const int my_rank) {
  ...
  printlnlog("propagation grid: {}-dimensional {}", get_ndim(GRID_TYPE), get_grid_type_name(GRID_TYPE));

  for (int d = 0; d < get_ndim(GRID_TYPE); d++) {
    printlnlog("    coordinate {} '{}': cells have {} position values", d, get_coordlabel(GRID_TYPE, d), ncoordgrid[d]);
  }
