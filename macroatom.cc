...
[[nodiscard]] auto do_macroatom_radrecomb(Packet& pkt, const int nonemptymgi, const int element, const int upperion,
  ...
  for (int tmp_lowerionlevel = 0; tmp_lowerionlevel < nlevels; tmp_lowerionlevel++) {
    ...
    const double R =
        rad_recombination_ratecoeff(T_e, clumpednne, element, upperion, upperionlevel, tmp_lowerionlevel);
    ...
    switch (selected_action) {
      ...
      case MA_ACTION_INTERNALDOWNLOWER: {
        ...
        for (int tmp_lower = 0; tmp_lower < nlevels; tmp_lower++) {
          ...
          const double C =
              col_recombination_ratecoeff(T_e, clumpednne, element, ion, level, tmp_lower, epsilon_trans);
...
[[gnu::pure]] [[nodiscard]] auto col_recombination_ratecoeff(const float T_e, float clumpednne, const int element,
  ...
  for (int phixstargetindex = 0; phixstargetindex < nphixstargets; phixstargetindex++) {
    if (get_phixsupperlevel(lowerionlower_uniquelevelindex, phixstargetindex) == upper) {
      ...
      const double C =
          clumpednne * clumpednne * SAHACONST * statw_lower / statw_upper * 1.55e13 * g * sigma_bf * KB / T_e / epsilon_trans;
...
// multiply by lower level population to get a rate per second
[[gnu::pure]] [[nodiscard]] auto col_ionisation_ratecoeff(const float T_e, float clumpednne, const int element, const int ion,
  ...
  const double C = clumpednne * 1.55e13 * pow(T_e, -0.5) * g * sigma_bf * exp(-fac1) / fac1;  // photoionisation at the edge
...
// multiply by upper level population to get a rate per second
[[gnu::pure]] [[nodiscard]] auto col_deexcitation_ratecoeff(const float T_e, float clumpednne, const double epsilon_trans,
                                                            const double upperstatweight, const double lowerstatweight,
                                                            const int alltransindex)
    -> double {
  ...
  if (coll_strength < 0) {
    if (!globals::alltrans.forbidden[alltransindex])  // alternative: (coll_strength > -1.5) i.e. to catch -1
    {
      ...
      return C_0 * 14.51039491 * clumpednne * std::sqrt(T_e) * trans_osc_strength * pow2(H_ionpot / epsilon_trans) * eoverkt *
             g_ratio * gauntfac;
    }
...
// multiply by lower level population to get a rate per second
[[gnu::pure]] [[nodiscard]] auto col_excitation_ratecoeff(const float T_e, float clumpednne, const double upperstatweight,
  ...
  if (coll_strength < 0) {
    ...
    if (!forbidden) {
      ...
      return C_0 * clumpednne * std::sqrt(T_e) * 14.51039491 * trans_osc_strength * pow2(H_ionpot / epsilon_trans) * eoverkt /
             exp_eoverkt * Gamma;
    }
  ...
  // from Osterbrock and Ferland, p51
  return clumpednne * 8.629e-6 * static_cast<double>(coll_strength) * std::exp(-eoverkt) / lowerstatweight / std::sqrt(T_e);
}
