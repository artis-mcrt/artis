...
static auto do_macroatom_internal_down_same(int element, int ion, int level) -> int {
  ...
  const double *sum_internal_down_same =
      globals::cellcache[slotid].chelements[element].chions[ion].chlevels[level].sum_internal_down_same;
...
static void do_macroatom_raddeexcitation(struct packet *pkt_ptr, const int element, const int ion, const int level,
  ...
  const auto *sum_epstrans_rad_deexc =
      globals::cellcache[slotid].chelements[element].chions[ion].chlevels[level].sum_epstrans_rad_deexc;
...
/// Material for handling activated macro atoms.
{
  ...
  while (!end_packet) {
    ...
    assert_testmodeonly(globals::cellcache[slotid].cellnumber == modelgridindex);

    auto &chlevel = globals::cellcache[slotid].chelements[element].chions[ion].chlevels[level];
    auto &processrates = chlevel.processrates;
    ...
    switch (selected_action) {
      ...
      case MA_ACTION_INTERNALUPSAME: {
        ...
        const double *sum_internal_up_same =
            globals::cellcache[slotid].chelements[element].chions[ion].chlevels[level].sum_internal_up_same;
