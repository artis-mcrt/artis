...
void place_pellet(const double e_cmf_per_packet, const std::span<const double> en_cumulative, const int pktnumber,
  ...
  if (grid::get_prop_gridtype() == GridType::SPHERICAL1D) {
  ...
  } else if (grid::get_prop_gridtype() == GridType::CYLINDRICAL2D) {
  ...
  } else if (grid::get_prop_gridtype() == GridType::CARTESIAN3D) {
