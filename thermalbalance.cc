...
{
  return globals::cellcache[cellcacheslotid].chelements[element].chions[ion].chlevels[level].bfheatingcoeff;
}
  ...
  for (int element = 0; element < get_nelements(); element++) {
    ...
    for (int ion = 0; ion < nions; ion++) {
      ...
      for (int level = 0; level < nlevels; level++) {
        ...
        globals::cellcache[cellcacheslotid].chelements[element].chions[ion].chlevels[level].bfheatingcoeff =
            bfheatingcoeff;
      }
    }
  }
  globals::cellcache[cellcacheslotid].bfheating_mgi = modelgridindex;
}
...
/// via the elements of the heatingrates data structure.
{
  ...
  assert_always(globals::cellcache[cellcacheslotid].bfheating_mgi == modelgridindex);
