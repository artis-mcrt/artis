...
static void write_deposition_file(const int nts, const int my_rank, const int nstart, const int ndo) {
  ...
  const int i = nts;
  {
    const double t_mid = globals::timestep[i].mid;

    // power in [erg/s]
    globals::timestep[i].eps_positron_ana_power = 0.;
    globals::timestep[i].eps_electron_ana_power = 0.;
    globals::timestep[i].eps_alpha_ana_power = 0.;
    globals::timestep[i].qdot_betaminus = 0.;
    globals::timestep[i].qdot_alpha = 0.;
    globals::timestep[i].qdot_total = 0.;
    ...
    // for (int mgi = 0; mgi < grid::get_npts_model(); mgi++)
    {
      if (grid::get_numassociatedcells(mgi) > 0) {
        ...
        for (const auto decaytype : decay::all_decaytypes) {
          ...
          globals::timestep[i].qdot_total += qdot_cell;
          if (decaytype == decay::DECAYTYPE_BETAMINUS) {
            globals::timestep[i].qdot_betaminus += qdot_cell;
          } else if (decaytype == decay::DECAYTYPE_ALPHA) {
            globals::timestep[i].qdot_alpha += qdot_cell;
          }
        }
      }
    }
    ...
    MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[i].eps_positron_ana_power, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[i].eps_electron_ana_power, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[i].eps_alpha_ana_power, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[i].qdot_betaminus, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[i].qdot_alpha, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[i].qdot_total, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#endif
  }
  ...
  if (my_rank == 0) {
    ...
    for (int i = 0; i <= nts; i++) {
      const double t_mid = globals::timestep[i].mid;
      const double t_width = globals::timestep[i].width;
      const double total_dep = (globals::timestep[i].gamma_dep + globals::timestep[i].positron_dep +
                                globals::timestep[i].electron_dep + globals::timestep[i].alpha_dep);

      // dep is used here for positrons and alphas because it is the same as the emission rate
      const double epsilon_mc = (globals::timestep[i].gamma_emission + globals::timestep[i].positron_dep +
                                 globals::timestep[i].electron_emission + globals::timestep[i].alpha_emission) /
                                mtot / t_width;

      fprintf(dep_file, "%d %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n", i, t_mid / DAY, t_mid,
              total_dep / t_width / LSUN, globals::timestep[i].gamma_dep / t_width / LSUN,
              globals::timestep[i].gamma_dep_pathint / t_width / LSUN,
              globals::timestep[i].positron_dep / t_width / LSUN, globals::timestep[i].eps_positron_ana_power / LSUN,
              globals::timestep[i].electron_dep / t_width / LSUN,
              globals::timestep[i].electron_emission / t_width / LSUN,
              globals::timestep[i].eps_electron_ana_power / LSUN, globals::timestep[i].alpha_dep / t_width / LSUN,
              globals::timestep[i].alpha_emission / t_width / LSUN, globals::timestep[i].eps_alpha_ana_power / LSUN,
              globals::timestep[i].gamma_emission / t_width / LSUN, globals::timestep[i].qdot_betaminus / mtot,
              globals::timestep[i].qdot_alpha / mtot, epsilon_mc, globals::timestep[i].qdot_total / mtot);
    }
    fclose(dep_file);
...
static void mpi_reduce_estimators(int nts) {
  ...
  MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[nts].cmf_lum, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[nts].gamma_dep, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[nts].positron_dep, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[nts].electron_dep, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[nts].electron_emission, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[nts].alpha_dep, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[nts].alpha_emission, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &globals::timestep[nts].gamma_emission, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

  globals::timestep[nts].cmf_lum /= globals::nprocs;
  globals::timestep[nts].gamma_dep /= globals::nprocs;
  globals::timestep[nts].positron_dep /= globals::nprocs;
  globals::timestep[nts].electron_dep /= globals::nprocs;
  globals::timestep[nts].electron_emission /= globals::nprocs;
  globals::timestep[nts].alpha_dep /= globals::nprocs;
  globals::timestep[nts].alpha_emission /= globals::nprocs;
  globals::timestep[nts].gamma_emission /= globals::nprocs;
...
/// Each process has now updated its own set of cells. The results now need to be communicated between processes.
  ...
  if ((nts < globals::timestep_finish) && do_this_full_loop) {
    ...
    printout("During timestep %d on MPI process %d, %d pellets decayed and %d packets escaped. (t=%gd)\n", nts, my_rank,
             globals::timestep[nts].pellet_decays, globals::nesc, globals::timestep[nts].mid / DAY);
