...
static int get_nuc_index_or_neg_one(int z, int a)
...
// get the nuclide array index from the atomic number and mass number
{
  const int nucindex = get_nuc_index_or_neg_one(z, a);
  if (nucindex >= 0) {
    return nucindex;
  }
...
static void printout_nuclidemeanlife(const int z, const int a) {
  ...
  if (exists && get_meanlife_bynucindex(nucindex) > 0.) {
    printout("[tau %.1es]", get_meanlife_bynucindex(nucindex));
  } else if (exists) {
...
static double get_nuc_decaybranchprob_bynucindex(const int nucindex, const int decaytype) {
  assert_testmodeonly(nucindex >= 0);
  assert_testmodeonly(nucindex < get_num_nuclides());
...
static double get_nuc_decaybranchprob(const int z_parent, const int a_parent, const int decaytype) {
...
static double calculate_decaypath_branchproduct(int decaypathindex)
...
static void find_decaypaths(void) {
  for (int startnucindex = 0; startnucindex < get_num_nuclides(); startnucindex++) {
    ...
    for (int dectypeindex = 0; dectypeindex < DECAYTYPE_COUNT; dectypeindex++) {
      if (get_nuc_decaybranchprob_bynucindex(startnucindex, dectypeindex) == 0. ||
          get_meanlife_bynucindex(startnucindex) <= 0.) {
        continue;
      }
...
static double sample_decaytime(const int decaypathindex, const double tdecaymin, const double tdecaymax) {
  ...
  while (tdecay <= tdecaymin || tdecay >= tdecaymax) {
    ...
    for (int i = 0; i < get_decaypathlength(decaypathindex); i++) {
      ...
      tdecay += -get_meanlife_bynucindex(nucindex) * log(zrand);
    }
  }
  return tdecay;
}

static double calculate_decaychain(const double firstinitabund, std::vector<double> &lambdas, const int num_nuclides,
...
static double get_nuc_massfrac(const int modelgridindex, const int z, const int a, const double time)
...
// For stable nuclides, abundance returned only comes from other decays (some could be included in init model elem frac)
{
  ...
  if (nuc_exists_z_a && get_meanlife_bynucindex(nucindex) <= 0.) {
    nuctotal += grid::get_modelinitradioabund_bynucindex(modelgridindex, nucindex);
  }

  return nuctotal;
}

static double get_endecay_to_tinf_per_ejectamass_at_time(const int modelgridindex, const int decaypathindex,
                                                         const double time)
...
double get_endecay_per_ejectamass_t0_to_time_withexpansion_chain_numerical(const int modelgridindex,
...
static double get_endecay_per_ejectamass_between_times(const int mgi, const int decaypathindex, double tlow,
                                                       double thigh)
...
static double get_decaypath_power_per_ejectamass(const int decaypathindex, const int modelgridindex, const double time)
