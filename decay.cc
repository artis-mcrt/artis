...
void setup_radioactive_pellet(const double e_cmf_per_packet, const int nonemptymgi, Packet& pkt,
  ...
  const double zrand_en = rng_uniform(rngstate(pkt)) * cumulative_en_sum[num_decaychannels - 1];
  ...
  if constexpr (UNIFORM_PELLET_ENERGIES) {
    pkt.tdecay = sample_decaytime(decaypathindex, tdecaymin, globals::tmax, rngstate(pkt));
    pkt.e_cmf = e_cmf_per_packet;
  } else {
    ...
    pkt.tdecay = std::lerp(globals::tmax, tdecaymin, rng_uniform(rngstate(pkt)));
  ...
  pkt.originated_from_particlenotgamma = (rng_uniform(rngstate(pkt)) >= engamma / (engamma + enparticle));
  if (pkt.originated_from_particlenotgamma) {
    // particle (positron, electron, or alpha) emitted
    pkt.nu_cmf = enparticle / H;
  } else {
    // gamma ray emitted
    pkt.nu_cmf = gammapkt::choose_gamma_ray(pkt.pellet_nucindex, rngstate(pkt));
  }
}
