...
extern double ENICKEL;
extern double ECOBALT;
extern double ECOBALT_GAMMA;
extern double E48CR;
extern double E48V;
...
extern int nxgrid, nygrid, nzgrid; /* actual grid dimensions to use */
extern int ngrid;
extern int grid_type, model_type;
...
//int nprocs_exspec;
extern int npkts;
extern int nesc; //number of packets that escape during current timestep  ///ATMOIC

extern double xmax, ymax, zmax;
extern double mtot, vmax, rmax; /* Total mass and outer velocity/radius. */
extern double fe_sum, rho_sum; //MK: could now be declared locally
extern double mni56; /*Total mass of Ni56 in the ejecta. */
extern double mco56; /*Total mass of Co56 in the ejecta. */
extern double mfe52; /*Total mass of Fe52 in the ejecta. */
extern double mcr48; /*Total mass of Cr48 in the ejecta. */
...
extern int itstep;       /// Inital timestep's number
...
extern double dlognusyn; //logarithmic interval for syn
...
struct gamma_spec
{
  ...
  int nlines;
} extern cobalt_spec, nickel_spec, fakeg_spec, cr48_spec, v48_spec;

extern LIST gam_line_list;
...
extern double DeltaA; /* area used for syn */ ///possible to do this as local variable?
...
extern double J[MMODELGRID+1];
extern double energy_deposition[MMODELGRID+1];
#ifdef DO_TITER
  extern double J_reduced_save[MMODELGRID+1];
#endif


#ifdef FORCE_LTE
  extern double redhelper[MMODELGRID+1];
#endif
#ifndef FORCE_LTE
  extern double nuJ[MMODELGRID+1];
  extern double ffheatingestimator[MMODELGRID+1];
  extern double colheatingestimator[MMODELGRID+1];
  extern double gammaestimator[(MMODELGRID+1)*MELEMENTS*MIONS];
  extern double bfheatingestimator[(MMODELGRID+1)*MELEMENTS*MIONS];
  extern double corrphotoionrenorm[(MMODELGRID+1)*MELEMENTS*MIONS];
  extern double redhelper[(MMODELGRID+1)*MELEMENTS*MIONS];


  #ifdef DO_TITER
    extern double nuJ_reduced_save[MMODELGRID];
  ...
  extern int *ecounter,*acounter,*linestat_reduced;
#endif


extern int do_comp_est; /* 1 = compute compton emissivity estimators. 0 = don't */
extern int do_r_lc;     /* If not set to 1 then the opacity for r-packets is 0. */
...
extern int file_set; /* 1 if the output files already exist. 0 otherwise. */

extern int npts_model; /*number of points in 1-D input model */
extern double vout_model[MMODELGRID], rho_model[MMODELGRID], fni_model[MMODELGRID], fco_model[MMODELGRID], ffegrp_model[MMODELGRID], f48cr_model[MMODELGRID],f52fe_model[MMODELGRID];
extern double abund_model[MMODELGRID][30];
...
/// ATOMIC DATA
///============================================================================
extern int nelements,nlines,includedions;
...
extern int importantcoolingterms;                /// Number of important cooling terms


extern coolingrates_t *coolingrates;


extern heatingrates_t *heatingrates;
//double heating_col;
...
extern int nbfcontinua,nbfcontinua_ground; ///number of bf-continua
...
extern int k_stat_from_gamma;
extern int k_stat_from_eminus;
...
extern int homogeneous_abundances;

#define NPHIXSPOINTS 100

///Constants for van-Regmorter approximation. Defined in input.c
extern double C_0;
extern double H_ionpot;

extern int continue_simulation;
extern int tid;
extern int nthreads;
...
extern int n_titer;
extern short initial_iteration;
...
extern transitions_t *transitions;
extern int maxion;
extern FILE *tau_file;
extern FILE *tb_file;
extern FILE *heating_file;
extern FILE *estimators_file;
